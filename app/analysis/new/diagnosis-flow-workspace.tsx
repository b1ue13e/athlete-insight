...
export function DiagnosisFlowWorkspace() {
  ...
  const handleCreateAthlete = async () => {
    ...
    setNewAthleteForm({ name: "", position: "主攻", team: "" })
  }
  ...
  const shellTitle =
    step === "input-data" && selectedAthlete
      ? `为 ${selectedAthlete.name} 开始录入`
      : step === "select-mode" && selectedAthlete
        ? `为 ${selectedAthlete.name} 选择录入模式`
        : step === "select-athlete"
          ? "排球诊断"
          : "新建诊断"
  ...
  const shellSubtitle =
    step === "input-data"
      ? `${inputMode === "quick" ? "快速模式" : "专业模式"} · ${selectedAthlete?.position ?? "排球专项"}`
      : step === "select-athlete"
        ? "先把这次分析归到正确的人名下"
        : step === "select-mode"
          ? "根据手头数据选择合适的填写深度"
  ...
  return (
    <WorkspaceShell
      ...
      actions={
        <>
          <button type="button" onClick={handleBack} className="action-secondary text-sm">
            <ArrowLeft className="h-4 w-4" />
            {step === "select-sport" ? "返回首页" : "上一步"}
          </button>
          <div className="data-pill text-xs uppercase tracking-[0.18em]">
            Step {String(Math.max(1, currentStepIndex + 1)).padStart(2, "0")} / {flowSteps.length}
          </div>
        </>
      }
    >
            ...
            {flowSteps.map((item, index) => {
              ...
              return (
                <div key={item.id} className="flex items-center gap-2">
                  <div
                    className={cn(
                      "inline-flex items-center gap-2 rounded-full border px-3 py-2 text-xs font-semibold uppercase tracking-[0.18em] transition-sharp",
                      isActive
                  ...
                  {index < flowSteps.length - 1 ? <div className="hidden h-px w-5 bg-[var(--line-default)] sm:block" /> : null}
                </div>
              )
            })}
          </div>
              ...
              <aside className="panel-elevated space-y-5 p-6">
                ...
                <div className="space-y-3">
                  {getStepTips(step).map((tip, index) => (
                    <div key={tip} className="rounded-2xl border border-[var(--line-default)] bg-[color-mix(in_oklch,var(--bg-secondary)_84%,transparent)] p-4">
              ...
              <section className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_320px]">
                ...
                <aside className="panel space-y-5 p-6">
                  ...
                  <div className="space-y-3 text-sm leading-6 text-[var(--text-secondary)]">
                    ...
                    <p>如果你要做排球专项诊断，下一步会继续进入运动员与模式选择工作台。</p>
                  </div>
                    ...
                    athletes.map((athlete) => (
                      ...
                      >
                        <div className="flex items-start justify-between gap-4">
                          <div className="space-y-2">
                            <div className={cn("text-xl font-semibold transition-sharp", selectedAthlete?.id === athlete.id ? "text-[var(--accent)]" : "text-[var(--text-primary)]")}>
                              {athlete.name}
                            </div>
                          ...
                          <ChevronRight className={cn("mt-1 h-5 w-5 transition-sharp", selectedAthlete?.id === athlete.id ? "translate-x-1 text-[var(--accent)]" : "text-[var(--text-muted)]")} />
                        </div>
                      </button>
                    ))
                  )}
                  ...
                  <div className="panel space-y-4 p-6">
                    ...
                    {!isCreatingAthlete && athletes.length > 0 ? (
                      <>
                        ...
                        <button type="button" onClick={() => setIsCreatingAthlete(true)} className="action-secondary w-full text-sm">
                      ...
                      <div className="space-y-4">
                        <Field label="姓名 *">
                          <input type="text" value={newAthleteForm.name} onChange={(event) => setNewAthleteForm((current) => ({ ...current, name: event.target.value }))} className="analysis-input" placeholder="输入姓名" />
                        </Field>

                        <Field label="位置">
                          <select
                            value={newAthleteForm.position}
                            onChange={(event) => setNewAthleteForm((current) => ({ ...current, position: event.target.value as VolleyballPosition }))}
                        ...
                        <Field label="队伍 / 组别">
                          <input type="text" value={newAthleteForm.team} onChange={(event) => setNewAthleteForm((current) => ({ ...current, team: event.target.value }))} className="analysis-input" placeholder="可选" />
                        </Field>

                        <div className="flex gap-3">
                          <button type="button" onClick={() => setIsCreatingAthlete(false)} className="action-secondary flex-1 text-sm">取消</button>
                          <button type="button" onClick={handleCreateAthlete} disabled={!newAthleteForm.name.trim()} className="action-primary flex-1 text-sm disabled:cursor-not-allowed disabled:opacity-40">创建</button>
                        </div>
                      </div>
                    )}
                  </div>

                  <button type="button" onClick={handleContinue} disabled={!selectedAthlete} className="action-primary w-full text-sm disabled:cursor-not-allowed disabled:opacity-40">
                    继续
                    <ChevronRight className="h-4 w-4" />
                  </button>
                </aside>
          ...
          <div className="space-y-8">
            <section className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-start">
              <div className="panel-elevated space-y-5 p-6 sm:p-8">
                ...
                <p className="max-w-2xl text-sm leading-7 text-[var(--text-secondary)] sm:text-base">
                  页面已经根据你前面的选择锁定了项目、对象和模式，接下来只需要专注完成判断与录入。
                </p>
                <div className="flex flex-wrap gap-3">
                  <div className="data-pill text-xs uppercase tracking-[0.18em]">
                    <User className="h-3.5 w-3.5 text-[var(--accent)]" />
                  ...
                  <div className="data-pill text-xs uppercase tracking-[0.18em]">排球分析</div>
                </div>
              </div>
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1 space-y-3">
          <div className="space-y-2">
            ...
            <div className={cn("text-xl font-semibold transition-sharp", selected ? "text-[var(--accent)]" : "text-[var(--text-primary)]")}>{title}</div>
